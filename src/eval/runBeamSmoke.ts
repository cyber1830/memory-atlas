...
async function main() {
  ...
  const chat = JSON.parse(
    fs.readFileSync(path.join(root, "chat.json"), "utf8"),
  );
  const probes = JSON.parse(
    fs.readFileSync(
      path.join(root, "probing_questions", "probing_questions.json"),
      "utf8",
    ),
  ) as Record<string, BeamProbe[]>;
  ...
  if (sourceIds.length)
    await waitForIngestion({ userId, sourceIds, timeoutMs: 10 * 60 * 1000 });

  const selected = Object.entries(probes).flatMap(([category, items]) =>
    items.slice(0, 1).map((item) => ({ ...item, category })),
  );
  const output = [];
  for (const probe of selected) {
    ...
    const answer =
      gate.verdict === "abstain"
        ? "[ABSTAIN]"
        : await generateAnswer(probe.question, facts);
    output.push({
      category: probe.category,
      question: probe.question,
      ideal_response: probe.ideal_response,
      answer,
