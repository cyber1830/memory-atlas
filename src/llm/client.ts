...
function factLine(f: RetrievedFact): string {
  ...
  const target =
    f.targetEntity.length > 700
      ? `${f.targetEntity.slice(0, 700)}…`
      : f.targetEntity;
...
export function extractiveEvidence(
  question: string,
  facts: RetrievedFact[],
): string {
  ...
  const ranked = candidates
    .map((item) => ({
      ...item,
      score:
        queryTerms.filter((term) => item.sentence.toLowerCase().includes(term))
          .length +
        item.fact.relevance * 0.1,
    }))
  ...
  const selected = [...bestPerFact.values()]
    ...
    .filter(
      (item, index, all) =>
        all.findIndex((candidate) => candidate.sentence === item.sentence) ===
        index,
    )
    .slice(0, 1);
  return selected.length
    ? selected.map((item) => item.sentence).join(" ")
    : compactFacts(facts).slice(0, 3).map(factLine).join("; ");
}
...
async function askOllama(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `[ollama] HTTP ${response.status} for model ${OLLAMA_MODEL}: ${body.slice(0, 300)}`,
      );
      return null;
    }
...
export async function checkOllama(): Promise<void> {
  if ((process.env.LLM_PROVIDER ?? "ollama") === "groq") {
    ...
    if (!response.ok)
      throw new Error(
        `Groq preflight failed (HTTP ${response.status}). Check GROQ_API_KEY.`,
      );
  ...
  if (!response.ok)
    throw new Error(`Ollama is unreachable (HTTP ${response.status})`);
  ...
  if (
    ...
    throw new Error(
      `Ollama model '${OLLAMA_MODEL}' is not installed. Available: ${models.join(", ") || "none"}`,
    );
  }
...
export async function generateAnswer(
  ...
  const looksLikeFragment = Boolean(
    answer &&
    (/^\s*[$€£]?\d+(?:\.\d+)?\s*$/.test(answer) ||
  ...
  if (
    answer &&
    !looksLikeFragment &&
    !/^\s*(i\s+don['’]?t\s+know|unknown|insufficient information)\.?\s*$/i.test(
      answer,
    )
  ) {
    return answer;
